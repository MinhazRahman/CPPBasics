...
public:
  ...
  double getPrice();
  int getQuantity();
  static void serialize(ofstream &, Item &);
  static ifstream &deserialize(ifstream &, Item &);
};
...
void Item::serialize(ofstream &ofs, Item &item)
{
  ofs << item.name << endl;
  ofs << item.price << endl;
...
ifstream &Item::deserialize(ifstream &ifs, Item &item)
{
